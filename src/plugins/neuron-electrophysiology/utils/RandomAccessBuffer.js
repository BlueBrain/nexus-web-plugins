...
class RandomAccessBuffer {
  ...
  parse(buff) {
    if (this._ioMode === IO_MODE.WRITE) {
      throw new Error(
        'This RandomAccessBuffer instance is accessible only in WRITE mode.'
      );
    }

    // checking the magic number
    if (
      String.fromCharCode.apply(null, new Uint8Array(buff, 0, 3)) !==
      MAGIC_NUMBER
    ) {
      throw new Error(
        'The buffer provided is not from a valid RandomAccessBuffer file.'
      );
    }
    ...
    try {
      this._rabIndex = yaml.safeLoad(textIndex);
    } catch (e) {
