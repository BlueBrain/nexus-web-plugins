...
export const App = (props: AppProps) => {
  ...
  const [currentPluginName, setCurrentPluginName] = useState<string>(config.pluginName);

  const [loading, setLoading] = useState(true);

  const [org, setOrg] = useState<string>(formData.org || '');
  const [project, setProject] = useState<string>(formData.project || '');
  const [resourceId, setResourceId] = useState<string>(formData.resourceId || '');
    ...
    <div className="app">
      <div className="plugin-ctrl">
        <div className="text-right">
          <p>
            Current Environment: <a href={nexus.context.uri}>{nexus.context.uri}</a>
          </p>
        </div>
        <Form layout="inline">
